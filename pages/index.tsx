...
import { IGaleryItem } from '../app/components/ui/galery/galery.interface'
...
export const getStaticProps: GetStaticProps = async () => {
	try {
		...
		const actors: IGaleryItem[] = dataActors.slice(0, 7).map((a) => ({
			name: a.name,
			posterPath: a.photo,
		...
		const trendingMovies: IGaleryItem[] = dataTrendingMovies
			.slice(0, 7)
			.map((m) => ({
				name: m.title,
				posterPath: m.poster,
